  ...
  case $::osfamily {
    'RedHat': {
      ...
      if $ver == '60' and $::operatingsystemmajrelease == '7' {
        ...
        fail('Varnish 6.0 from Packagecloud is not supported on OS release under 7)')
      }
      if $ver == '61' and $::operatingsystemmajrelease == '7' {
      } else {
        fail('Varnish 6.1 from Packagecloud is not supported on OS release under 7)')
      }
    ...
    'Debian': {
      ...
      if $ver == '60' {
        if !($::lsbdistcodename == 'stretch') or !($::lsbdistcodename == 'bionic') or !($::lsbdistcodename == 'xenial') {
          ...
          fail('Varnish 6.0 from Packagecloud is not supported on this Debian version')
        }
      }
      if $ver == '61' {
        if !($::lsbdistcodename == 'stretch') or !($::lsbdistcodename == 'bionic') or !($::lsbdistcodename == 'xenial') {
        } else {
          fail('Varnish 6.1 from Packagecloud is not supported on this Debian version')
        }
      }
